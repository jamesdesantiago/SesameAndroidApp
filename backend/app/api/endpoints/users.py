...
async def unfollow_user(
    ...
    # Implementation unchanged from previous version
    try:
        # crud_user.unfollow_user returns True if deleted, False if not found (on the relationship).
        # It raises DatabaseInteractionError for DB issues.
        deleted = await crud_user.unfollow_user(db=db, follower_id=current_user_id, followed_id=user_id)
        if not deleted:
            # If no relationship was deleted (because user wasn't following),
            # we still need to check if the *target user* exists to return the
            # correct 404 (user not found) vs 200 OK (not following).
            target_exists = await crud_user.check_user_exists(db=db, user_id=user_id)
            if not target_exists:
                 # If target user doesn't exist, it's a 404
                 logger.warning(f"User {current_user_id} attempted to unfollow non-existent user {user_id}")
                 raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User to unfollow not found")
            else:
                 # If target user exists but no relationship was deleted, it means user_id wasn't being followed
                 logger.info(f"User {current_user_id} attempted to unfollow user {user_id}, but no follow relationship found.")
                 return user_schemas.UsernameSetResponse(message="Not following this user")
        # If deleted is True, the relationship was successfully removed
    ...
    except DatabaseInteractionError as e: # Catch generic DB errors from unfollow_user
