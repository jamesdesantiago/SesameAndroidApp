...
# test_check_username_needs_it now uses create_test_user_direct from utils and db_tx
async def test_check_username_needs_it(client: AsyncClient, test_user1: Dict[str, Any], db_tx: asyncpg.Connection, mock_auth):
    ...
    # Use db_tx and create_test_user_direct from utils
    # test_user1 fixture already creates the user via create_test_user_direct(db_tx, ...)
    # We just need to explicitly set the username to NULL for this specific test scenario
    await db_tx.execute("UPDATE users SET username = NULL WHERE id = $1", test_user1["id"])
...
# test_check_username_does_not_need_it now uses create_test_user_direct from utils and db_tx
async def test_check_username_does_not_need_it(client: AsyncClient, test_user1: Dict[str, Any], db_tx: asyncpg.Connection, mock_auth):
    ...
    await db_tx.execute("UPDATE users SET username = $1 WHERE id = $2", test_user1["username"], test_user1["id"])
    # mock_auth fixture handles auth
    response = await client.get(f"{API_V1}/users/check-username")
    assert response.status_code == status.HTTP_200_OK
    assert response.json() == {"needsUsername": False}

# test_set_username_valid now uses create_test_user_direct from utils and db_tx
async def test_set_username_valid(client: AsyncClient, test_user1: Dict[str, Any], mock_auth, db_tx: asyncpg.Connection):
    ...
    db_username = await db_tx.fetchval("SELECT username FROM users WHERE id = $1", test_user1["id"])
...
# test_set_username_conflict now uses create_test_user_direct from utils and db_tx
async def test_set_username_conflict(client: AsyncClient, test_user1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection, mock_auth):
    ...
    await db_tx.execute("UPDATE users SET username = $1 WHERE id = $2", conflicting_username, test_user2["id"])
...
# test_get_following_pagination now uses create_test_user_direct and create_follow_direct from utils and db_tx
async def test_get_following_pagination(client: AsyncClient, test_user1: Dict[str, Any], db_tx: asyncpg.Connection, mock_auth):
    """Test /users/following - Pagination logic."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # mock_auth handles auth for test_user1 (the follower)
    follower_id = test_user1["id"]
    followed_users = []
    # Arrange: Create users and follow relationships using imported helpers and db_tx
    for i in range(5):
        # Use imported helper
        user = await create_test_user_direct(db_tx, f"following_tgt_{i}_tx")
        followed_users.append(user)
        # Use imported helper
        await create_follow_direct(db_tx, follower_id=follower_id, followed_id=user["id"])
...
# test_get_followers_pagination_and_following_flag now uses create_test_user_direct and create_follow_direct from utils and db_tx
async def test_get_followers_pagination_and_following_flag(client: AsyncClient, test_user1: Dict[str, Any], db_tx: asyncpg.Connection, mock_auth):
    ...
    # Arrange: Create follower users and relationships using imported helpers and db_tx
    for i in range(4):
        follower = await create_test_user_direct(db_tx, f"follower_{i}_tx")
        followers.append(follower)
        await create_follow_direct(db_tx, follower_id=follower["id"], followed_id=user_id) # This user follows user_id
        if i % 2 == 0:
             # user_id follows this follower back
             await create_follow_direct(db_tx, follower_id=user_id, followed_id=follower["id"])
...
# test_search_users_pagination now uses create_test_user_direct from utils and db_tx
async def test_search_users_pagination(client: AsyncClient, test_user1: Dict[str, Any], db_tx: asyncpg.Connection, mock_auth):
    ...
    matching_users = []
    non_matching = None
    # Arrange: Create users using imported helpers and db_tx
    for i in range(3):
        user = await create_test_user_direct(db_tx, f"searchme_{i}_tx")
        matching_users.append(user)
    non_matching = await create_test_user_direct(db_tx, "dontfindme_tx")
...
# test_follow_user_already_following now uses create_follow_direct from utils and db_tx
async def test_follow_user_already_following(client: AsyncClient, test_user1, test_user2, db_tx: asyncpg.Connection, mock_auth):
    """Test POST /users/{user_id}/follow - Already following returns 200 OK."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # mock_auth handles auth for test_user1 (the follower)
    user_to_follow_id = test_user2["id"]
    # Arrange: Create follow relationship using imported helper and db_tx
    await create_follow_direct(db_tx, follower_id=test_user1["id"], followed_id=user_to_follow_id)
...
# test_unfollow_user_success now uses create_follow_direct from utils and db_tx
async def test_unfollow_user_success(client: AsyncClient, test_user1, test_user2, db_tx: asyncpg.Connection, mock_auth):
    """Test DELETE /users/{user_id}/follow - Success."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # mock_auth handles auth for test_user1 (the unfollower)
    user_to_unfollow_id = test_user2["id"]
    # Arrange: Create follow relationship using imported helper and db_tx
    await create_follow_direct(db_tx, follower_id=test_user1["id"], followed_id=user_to_unfollow_id)
    # Verify relationship exists initially
    exists = await db_tx.fetchval("SELECT EXISTS(SELECT 1 FROM user_follows WHERE follower_id=$1 AND followed_id=$2)", test_user1["id"], user_to_unfollow_id)
    ...
    exists_after = await db_tx.fetchval("SELECT EXISTS(SELECT 1 FROM user_follows WHERE follower_id=$1 AND followed_id=$2)", test_user1["id"], user_to_unfollow_id)
...
# test_get_notifications_pagination now uses create_notification_direct from utils and db_tx
async def test_get_notifications_pagination(client: AsyncClient, test_user1, db_tx: asyncpg.Connection, mock_auth):
    """Test GET /notifications - Pagination logic."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # mock_auth handles auth for test_user1
    user_id = test_user1["id"]
    notif_ids = []
    # Arrange: Create notifications using imported helper and db_tx
    for i in range(5):
        # Use imported helper
        notif_data = await create_notification_direct(db_tx, user_id, f"Title {i}", f"Message {i}")
    ...
    assert len(data1["items"]) == 3
    ids1 = {item["id"] for item in data1["items"]}
    # Corrected assertion for order (assuming 5 notifications created sequentially and ordered DESC)
    ...
    # Page 2 (size 3): N1, N0 --> IDs: notif_ids[1], notif_ids[0]
