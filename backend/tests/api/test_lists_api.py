...
# async def create_test_place_direct(...): ... # REMOVED
# async def add_collaborator_direct(...): ... # REMOVED
# async def create_test_user_direct(...): ... # REMOVED
...
async def test_create_list_success(client: AsyncClient, mock_auth, test_user1: Dict[str, Any], is_private: bool, db_tx: asyncpg.Connection):
    ...
    db_list = await db_tx.fetchrow("SELECT owner_id, name, description, is_private FROM lists WHERE id = $1", list_id)
    ...
    # Cleanup is handled by db_tx fixture transaction rollback
...
async def test_get_user_lists_pagination(client: AsyncClient, db_tx: asyncpg.Connection, mock_auth, test_user1: Dict[str, Any], test_user2: Dict[str, Any]):
    ...
    try:
        for i in range(3): # User1 has 3 lists
            ...
            lst = await create_test_list_direct(db_tx, test_user1["id"], f"U1 List {i} {os.urandom(2).hex()}", i % 2 == 0)
            user1_lists.append(lst)
        # User2 has 1 list (should not be returned)
        # Use imported helper
        user2_list = await create_test_list_direct(db_tx, test_user2["id"], "U2 List 0", False)
        ...
        # Cleanup handled by db_tx fixture transaction rollback
        pass # Explicit pass since cleanup is handled

async def test_get_list_detail_success_owner(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], test_user1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test GET /lists/{list_id} - Success for owner, includes collaborators."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # Arrange: Add collaborator using imported helper
    await add_collaborator_direct(db_tx, test_list1["id"], test_user2["id"])
...
async def test_get_list_detail_success_collaborator(client: AsyncClient, test_user1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test GET /lists/{list_id} - Success for collaborator on private list."""
    # This test requires the DB pool initialized, db_tx working.
    # Arrange: Create a private list owned by user1, add user2 as collaborator
    private_list = await create_test_list_direct(db_tx, test_user1["id"], "Collab Test Private List", True)
    list_id = private_list["id"]
    await add_collaborator_direct(db_tx, list_id, test_user2["id"])

    # Mock auth for user2 (the collaborator)
    # Use the correct path for deps.get_verified_token_data
    ...
    app.dependency_overrides["app.api.deps.get_verified_token_data"] = override_auth_user2
...
async def test_get_list_detail_forbidden(client: AsyncClient, mock_auth, test_user1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test GET /lists/{list_id} - Forbidden for non-owner/collaborator of private list."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # Arrange: Create a private list owned by user2
    list_data = await create_test_list_direct(db_tx, test_user2["id"], "Other User Private List", True)
...
async def test_update_list_success(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test PATCH /lists/{list_id} - Success."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # Arrange: Ensure test_list1 is public initially
    list_id = test_list1["id"]
    await db_tx.execute("UPDATE lists SET is_private = FALSE WHERE id = $1", list_id)
    # mock_auth fixture handles auth for test_user1 (the owner)
    ...
    # Verify in DB
    db_list = await db_tx.fetchrow("SELECT name, is_private FROM lists WHERE id = $1", list_id)
    assert db_list["name"] == new_name
    assert db_list["is_private"] is True

async def test_update_list_forbidden(client: AsyncClient, mock_auth, test_user1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test PATCH /lists/{list_id} - Non-owner cannot update."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # Arrange: Create list owned by user2
    list_data = await create_test_list_direct(db_tx, test_user2["id"], "Another User List", False)
    list_id = list_data["id"]
...
async def test_delete_list_success(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test DELETE /lists/{list_id} - Success."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    list_id = test_list1["id"]
    # Arrange: Add a place to ensure cascade works
    await create_test_place_direct(db_tx, list_id, "Place in deleted list", "Addr", f"ext_del_list_{os.urandom(3).hex()}")
    assert await db_tx.fetchval("SELECT EXISTS (SELECT 1 FROM lists WHERE id = $1)", list_id)
    assert await db_tx.fetchval("SELECT COUNT(*) FROM places WHERE list_id = $1", list_id) > 0
    ...
    assert not await db_tx.fetchval("SELECT EXISTS (SELECT 1 FROM lists WHERE id = $1)", list_id)
    assert await db_tx.fetchval("SELECT COUNT(*) FROM places WHERE list_id = $1", list_id) == 0 # Verify cascade

async def test_delete_list_forbidden(client: AsyncClient, mock_auth, test_user1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test DELETE /lists/{list_id} - Non-owner cannot delete."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # Arrange: Create list owned by user2
    list_data = await create_test_list_direct(db_tx, test_user2["id"], "Another User List To Delete", False)
    list_id = list_data["id"]
...
# =====================================================
async def test_add_collaborator_success(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
    ...
    # Verify in DB
    is_collab = await db_tx.fetchval("SELECT EXISTS(SELECT 1 FROM list_collaborators WHERE list_id = $1 AND user_id = $2)", list_id, test_user2["id"])
...
async def test_add_collaborator_already_exists(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
...
async def test_add_collaborator_owner_is_collaborator(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], db_tx: asyncpg.Connection):
    ...
    # Assert
    # CRUD now raises CollaboratorAlreadyExistsError (or similar) if owner email is used
    assert response.status_code == status.HTTP_409_CONFLICT # Expect conflict if attempting to add owner
    assert "owner" in response.json()["detail"].lower() # Check detail message

async def test_delete_collaborator_success(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
    ...
    # Arrange: Add first using imported helper
    await add_collaborator_direct(db_tx, list_id, collaborator_id_to_remove)
    assert await db_tx.fetchval("SELECT EXISTS (SELECT 1 FROM list_collaborators WHERE list_id = $1 AND user_id = $2)", list_id, collaborator_id_to_remove)
    ...
    # Verify deleted in DB
    assert not await db_tx.fetchval("SELECT EXISTS (SELECT 1 FROM list_collaborators WHERE list_id = $1 AND user_id = $2)", list_id, collaborator_id_to_remove)

async def test_delete_collaborator_forbidden(client: AsyncClient, test_user1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
     ...
     list_data = await create_test_list_direct(db_tx, test_user1["id"], "List for Collab Deletion", False)
     list_id = list_data["id"]
     await add_collaborator_direct(db_tx, list_id, test_user2["id"])
...
async def test_delete_collaborator_not_found(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], db_tx: asyncpg.Connection):
    ...
    non_collab_user = await create_test_user_direct(db_tx, "non_collab_del_tx")
    non_collab_user_id = non_collab_user["id"]

    # Ensure this user is NOT a collaborator (default state from creation, but double-check)
    await db_tx.execute("DELETE FROM list_collaborators WHERE list_id = $1 AND user_id = $2", list_id, non_collab_user_id) # Ensure no relationship

    # Act
    response = await client.delete(f"{API_V1_LISTS}/{list_id}/collaborators/{non_collab_user_id}")

    # Assert
    # API now checks if user exists before returning 404
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert "User is not a collaborator on this list" in response.json()["detail"] # Corrected detail based on code
...
async def test_delete_collaborator_non_existent_user(client: AsyncClient, mock_auth, test_list1: Dict[str, Any]):
    ...
    # Assert
    # API now checks if user exists and returns 404
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert "Collaborator user not found" in response.json()["detail"] # Corrected detail based on code
...
async def test_delete_collaborator_owner_cannot_remove_self(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], test_user1: Dict[str, Any]):
    ...
    # Assert
    # API now checks if the user is the owner and returns 400 BAD REQUEST
...
async def test_add_and_get_places_in_list(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test POST + GET /{list_id}/places - Add and retrieve places with pagination."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # mock_auth handles auth for test_user1 (owner)
    list_id = test_list1["id"]
    places_created = []
    # Arrange: Add two places using imported helper and db_tx
...
async def test_update_place_notes_success(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test PATCH /{list_id}/places/{place_id} - Success updating notes."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # mock_auth handles auth for test_user1 (owner)
    list_id = test_list1["id"]
    # Arrange: Create a place using imported helper and db_tx
    place_data = await create_test_place_direct(db_tx, list_id, "Place To Update Notes", "Addr", f"ext_upd_notes_{os.urandom(3).hex()}", notes="Initial notes.")
    ...
    # Verify in DB
    db_place = await db_tx.fetchrow("SELECT notes FROM places WHERE id = $1", place_db_id)
    assert db_place["notes"] == new_notes

async def test_update_place_partial_success(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], db_tx: asyncpg.Connection):
    ...
    place_data = await create_test_place_direct(db_tx, list_id, "Place Partial Update", "Addr", f"ext_partial_upd_{os.urandom(3).hex()}", notes="Initial notes.", rating="MUST_VISIT")
    place_db_id = place_data["id"]
    ...
    # Verify in DB
    db_place = await db_tx.fetchrow("SELECT notes, rating FROM places WHERE id = $1", place_db_id)
    assert db_place["notes"] == new_notes
    assert db_place["rating"] == place_data["rating"]


async def test_update_place_forbidden(client: AsyncClient, test_user1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
     ...
     list_data = await create_test_list_direct(db_tx, test_user2["id"], "List Other User Update Place", False)
     place_data = await create_test_place_direct(db_tx, list_data["id"], "Other Place", "Addr", f"ext_other_upd_{os.urandom(3).hex()}")
     list_id = list_data["id"]
     place_db_id = place_data["id"]
     ...
     app.dependency_overrides["app.api.deps.get_verified_token_data"] = override_auth
     ...
     # Assert
     assert response.status_code == status.HTTP_403_FORBIDDEN # get_list_and_verify_access dependency fails


async def test_delete_place_success(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], db_tx: asyncpg.Connection):
     """Test DELETE /{list_id}/places/{place_id} - Success."""
     # This test requires the DB pool initialized, db_tx working, and mock_auth working.
     # mock_auth handles auth for test_user1 (owner)
     list_id = test_list1["id"]
     # Arrange: Create a place using imported helper and db_tx
     place_data = await create_test_place_direct(db_tx, list_id, "Place To Delete", "Addr", f"ext_del_{os.urandom(3).hex()}")
     place_db_id = place_data["id"]
     # Verify place exists initially
     assert await db_tx.fetchval("SELECT EXISTS (SELECT 1 FROM places WHERE id=$1 AND list_id=$2)", place_db_id, list_id)
     ...
     assert del_response.status_code == status.HTTP_204_NO_CONTENT
     # Verify deleted in DB
     assert not await db_tx.fetchval("SELECT EXISTS (SELECT 1 FROM places WHERE id=$1 AND list_id=$2)", place_db_id, list_id)

async def test_delete_place_forbidden(client: AsyncClient, test_user1: Dict[str, Any], test_user2: Dict[str, Any], db_tx: asyncpg.Connection):
     """Test DELETE /{list_id}/places/{place_id} - Forbidden for non-owner/collaborator."""
     # This test requires the DB pool initialized, db_tx working.
     # Arrange: Create list owned by user2, add a place to it
     list_data = await create_test_list_direct(db_tx, test_user2["id"], "List Other User Delete Place", False)
     place_data = await create_test_place_direct(db_tx, list_data["id"], "Other Place Del", "Addr", f"ext_other_del_{os.urandom(3).hex()}")
     list_id = list_data["id"]
     place_db_id = place_data["id"]
...
async def test_delete_place_not_found_in_list(client: AsyncClient, mock_auth, test_list1: Dict[str, Any], db_tx: asyncpg.Connection):
    """Test DELETE /{list_id}/places/{place_id} - Place ID exists, but not in THIS list."""
    # This test requires the DB pool initialized, db_tx working, and mock_auth working.
    # mock_auth handles auth for test_user1 (owner)
    list_id = test_list1["id"]
    # Arrange: Create a place in a DIFFERENT list
    list_other = await create_test_list_direct(db_tx, test_user1["id"], "Other list", False)
    place_data_other_list = await create_test_place_direct(db_tx, list_other["id"], "Place in other list", "Addr", f"ext_other_list_{os.urandom(3).hex()}")
    ...
    # Cleanup of list_other and its place is handled by db_tx transaction rollback
