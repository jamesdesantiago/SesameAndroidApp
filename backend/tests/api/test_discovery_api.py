...
async def test_get_public_lists_empty(client: AsyncClient):
    ...
    # The client fixture uses db_tx, which depends on db_conn, which depends on lifespan_db_pool_manager.
    # If lifespan_db_pool_manager fails, the client fixture setup fails, resulting in the "DB Pool not available" error.
    # This test itself doesn't directly call the database, but relies on the client being able to connect via its dependencies.
...
async def test_get_public_lists_success(client: AsyncClient, db_tx: asyncpg.Connection, test_user1, test_user2):
    ...
    # Use db_tx here
    public_list1 = await create_test_list_direct(db_tx, test_user1["id"], "Public List 1", False)
    private_list1 = await create_test_list_direct(db_tx, test_user1["id"], "User1 Private List", True)
    public_list2 = await create_test_list_direct(db_tx, test_user2["id"], "Public List 2", False)
...
async def test_search_lists_unauthenticated(client: AsyncClient, db_tx: asyncpg.Connection, test_user1, test_user2, mock_auth_optional_unauthenticated):
    """Test searching lists without authentication (should only find public)."""
    # This test requires the DB pool initialized and the db_tx fixture working.
    # Arrange: Create lists using the direct helper
    pub1 = await create_test_list_direct(db_tx, test_user1["id"], "Search Public Alpha", False, description="Contains target")
    priv1 = await create_test_list_direct(db_tx, test_user1["id"], "Search Private Alpha", True)
    pub2 = await create_test_list_direct(db_tx, test_user2["id"], "Another Public Search", False)
    priv2 = await create_test_list_direct(db_tx, test_user2["id"], "Other Private Beta", True) # Search term "Alpha" won't match Beta
...
async def test_search_lists_authenticated(client: AsyncClient, db_tx: asyncpg.Connection, test_user1, test_user2, mock_auth_optional):
    ...
    # Arrange: Create lists using the direct helper
    pub1 = await create_test_list_direct(db_tx, test_user1["id"], "My Search Public Alpha", False, description="Contains target")
    priv1 = await create_test_list_direct(db_tx, test_user1["id"], "My Search Private Alpha", True)
    pub2 = await create_test_list_direct(db_tx, test_user2["id"], "Other Public Search Beta", False) # Contains "Search" and "Beta"
    priv2 = await create_test_list_direct(db_tx, test_user2["id"], "Other Private Beta", True) # Contains "Beta"
...
async def test_get_recent_lists_success(client: AsyncClient, db_tx: asyncpg.Connection, test_user1, test_user2, mock_auth):
    ...
    # Use db_tx here
    priv2 = await create_test_list_direct(db_tx, test_user2["id"], "Other Private Recent", True)
    await asyncio.sleep(0.01) # Ensure distinct timestamps
    pub2 = await create_test_list_direct(db_tx, test_user2["id"], "Other Public Recent", False)
    await asyncio.sleep(0.01)
    priv1 = await create_test_list_direct(db_tx, test_user1["id"], "My Private Recent", True)
    await asyncio.sleep(0.01)
    pub1 = await create_test_list_direct(db_tx, test_user1["id"], "My Public Recent", False)
